...
class Player {
  ...
  get denomText() {
    if (this.stripes) return '条纹';
    if (this.solids) return '全色';
    return '';
  }

  get invalidContactText() {
    if (this.stripes) return `${this.nameText} 没有先击中条纹球。`;
    if (this.solids) return `${this.nameText} 没有先击中全色球。`;
  }

  get nameText() {
    if (this.number === 1) return '<strong>您</strong>';
  ...
  get winText() {
    if (this.number === 1) return '<strong>您</strong> 赢了!但这并不能证明你的水平';
    return '<strong>AI</strong> 赢了!你也太菜了吧!';
  }
